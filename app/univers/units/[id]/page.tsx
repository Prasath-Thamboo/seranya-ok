...
const UniversPage = () => {
  ...
  useEffect(() => {
    ...
    const loadRandomBackgrounds = async () => {
      try {
        const pageBackground = await fetchRandomBackground();
        const heroBackground = await fetchRandomBackground();
        setBackgroundImage(pageBackground);
        setHeroBackgroundImage(heroBackground);
        console.log("Page Background Image:", pageBackground);
        console.log("Hero Background Image:", heroBackground);
      } catch (error) {
        console.error("Échec du chargement des images de fond aléatoires :", error);
        setBackgroundImage("/images/backgrounds/default.jpg"); // Image de secours
        setHeroBackgroundImage("/images/backgrounds/default-hero.jpg"); // Image de secours
      }
    };
  ...
  // Helper function pour transformer les classes pour BadgeComponent
      ...
      {backgroundImage && (
        <div className="fixed inset-0 z-0">
          <Image
            ...
            priority={true}
            quality={100}
            className="brightness-50" // Ajustement de la luminosité
          />
          {/* Overlay pour assombrir l'image */}
          <div className="absolute inset-0 bg-black opacity-30 z-0"></div> {/* Opacité réduite */}
        </div>
      )}
