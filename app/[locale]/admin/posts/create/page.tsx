...
const CreatePost = () => {
  ...
  const handleSubmit = async (values: any) => {
    ...
    try {
      ...
      // Version anglaise (optionnelle — repli FR côté backend si vide)
      if (values.titleEn) formData.append('titleEn', values.titleEn);
      if (values.introEn) formData.append('introEn', values.introEn);
      if (values.subtitleEn) formData.append('subtitleEn', values.subtitleEn);
      if (contentEnValue) formData.append('contentEn', contentEnValue);
      ...
      <div className="bg-white p-8 rounded-lg shadow-lg w-full max-w-2xl">
        ...
        >
          ...
          <div className="mb-4 rounded-md border border-dashed border-gray-300 p-4">
            ...
            <Form.Item
              name="titleEn"
              label={<span className="font-sans text-ink-soft">Title (EN)</span>}
            >
              <Input placeholder="Post title" className="bg-raised text-ink font-sans" style={{ height: '3rem' }} />
            </Form.Item>

            <Form.Item
              name="introEn"
              label={<span className="font-sans text-ink-soft">Introduction (EN)</span>}
            >
              <Input.TextArea placeholder="Introduction" className="bg-raised text-ink font-sans" style={{ height: '6rem' }} />
            </Form.Item>

            <Form.Item
              name="subtitleEn"
              label={<span className="font-sans text-ink-soft">Subtitle (EN)</span>}
            >
              <Input placeholder="Post subtitle" className="bg-raised text-ink font-sans" style={{ height: '3rem' }} />
            </Form.Item>

            <Form.Item
              name="contentEn"
              label={<span className="font-sans text-ink-soft">Content (EN)</span>}
              className="mb-0"
            >
              <ReactQuill value={contentEnValue} onChange={setContentEnValue} />
            </Form.Item>
          </div>
